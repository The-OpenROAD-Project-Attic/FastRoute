...
void mazeMSMD(int netID, int enlarge, float d1[][XRANGE], float d2[][XRANGE], int **parentX1, int **parentY1, int **parentX3, int **parentY3, float **heap1, float **heap2,  Bool pop_heap2[], int corrEdge[][XRANGE], int ripup_threshold, int mazeedge_Threshold)
{
...
void mazeRouteMSMD(int expand, float costHeight, int ripup_threshold, int mazeedge_Threshold, Bool Ordering, int cost_type)
{
	...
	if (cost_type == 2) {
		for(i=0; i<forange; i++) {
			if(i<hCapacity-1)
				...
				h_costTable[i] = costHeight/(exp((float)(hCapacity-i-1)*LOGIS_COF)+1) + 1 + L*costHeight/slope*(i-hCapacity+1) ;
		}
		...
		for(i=0; i<forange; i++) {
			if(i<vCapacity-1)
				...
				v_costTable[i] = costHeight/(exp((float)(vCapacity-i-1)*LOGIS_COF)+1) + 1 + L*costHeight/slope*(i-vCapacity+1) ;
		} 
	}if (cost_type == 3) {
		for(i=0; i<forange; i++) {
			if(i<=hCapacity)
	...
	if (Ordering) {
		...
		for(i=0; i<numValidNets; i++)
		{
			...
			mazeMSMD(netID, expand, d1, d2, parentX1, parentY1, parentX2, parentY2, heap1, heap2,  pop_heap2, corrEdge, ripup_threshold, mazeedge_Threshold);
		}
	} else {
		for(i=0; i<numValidNets; i++)
		{
			mazeMSMD(i, expand, d1, d2, parentX1, parentY1, parentX2, parentY2, heap1, heap2,  pop_heap2, corrEdge, ripup_threshold, mazeedge_Threshold);
		}
	}
