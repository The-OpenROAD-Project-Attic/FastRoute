...
void readFile(char benchFile[])
{
	...
	v_st = (short**) malloc (yGrid*sizeof(short *));
	v_nd = (short**) malloc (yGrid*sizeof(short *));
	h_st = (short**) malloc (yGrid*sizeof(short *));
	h_nd = (short**) malloc (yGrid*sizeof(short *));

	v_int = (short***) malloc (yGrid*sizeof(short **));
	h_int = (short***) malloc (yGrid*sizeof(short **));

	for (i = 0; i < yGrid; i++) {
		v_st[i] = (short*) malloc (xGrid*sizeof(short));
		v_nd[i] = (short*) malloc (xGrid*sizeof(short));
		h_st[i] = (short*) malloc (xGrid*sizeof(short));
		h_nd[i] = (short*) malloc (xGrid*sizeof(short));
		v_int[i] = (short**) malloc (xGrid*sizeof(short *));
		h_int[i] = (short**) malloc (xGrid*sizeof(short *));
	}

	for (i = 0; i < yGrid; i++) {
		for (j = 0; j < xGrid; j++) {
			h_int[i][j] = (short*) malloc ((xGrid-j)*sizeof(short));
		}
	}

	for (i = 0; i < yGrid; i++) {
		for (j = 0; j < xGrid; j++) {
			v_int[i][j] = (short*) malloc ((yGrid-i)*sizeof(short));
		}
	}
