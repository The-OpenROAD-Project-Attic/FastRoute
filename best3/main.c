...
//    char benchFile[FILESTRLEN];
    ...
    int ESTEP3,CSTEP3, thStep3, tUsage;
   ...
   if(argc)
   {
    ...
    if(input==1)
	{
		...
		for (i = 1; i <= LVIter; i++) {
			...
			printf("LV routing round %d, enlarge %d LOGIS_COF %f\n", i,enlarge,LOGIS_COF);
	...
	//	past_cong = getOverflow2Dmaze( &maxOverflow); 
		...
		while(totalOverflow>0)
		{
			...
			if(totalOverflow>15000) {
				enlarge+= max(xGrid,yGrid) / 20;
				if (i == 5) {
					VIA = 0;
					LOGIS_COF = 1.33;
					ripup_threshold = -1;
			...
			printf("iteration %d, enlarge %d, costheight %d, threshold %d via cost %d \nlog_coef %f, healingTrigger %d cost_step %d L %d cost_type %d\n",i,enlarge,costheight,mazeedge_Threshold, VIA,LOGIS_COF, healingTrigger, cost_step, L ,cost_type);
			mazeRouteMSMD(enlarge, costheight, ripup_threshold,mazeedge_Threshold, !(i%3), cost_type);
			last_cong = past_cong;
			past_cong = getOverflow2Dmaze(&maxOverflow , & tUsage); 
