...
DEFControlParser::DEFControlParser() {}  // end constructor

// -----------------------------------------------------------------------------

DEFControlParser::~DEFControlParser() {}  // end destructor
...
int defGCellGrid(defrCallbackType_e typ, defiGcellGrid* gCell,
                 defiUserData data);
...
int defGroupMember(defrCallbackType_e type, const char* name,
...
char* defAddress(const char* in) { return ((char*)in); }  // end method
...
int defPin(defrCallbackType_e, defiPin* pin, defiUserData userData) {
        ...
        if (pin->hasLayer()) {
                ...
                defPin.clsICCADPos +=
                    defPin.clsLayerBounds.computeCenter();  // legacy iccad15
                                                            // contest pin
                                                            // position
        }                                                   // end if
        return 0;
...
int defNet(defrCallbackType_e c, defiNet* net, defiUserData ud) {
        ...
        for (int i = 0; i < net->numWires(); i++) {
                ...
                for (int j = 0; j < wire->numPaths(); j++) {
                        DefWireSegmentDscp& segmentDscp =
                            wireDscp.clsWireSegments[j];
                        ...
                        while (pathId != DEFIPATH_DONE) {
                                switch (pathId) {
                                        ...
                                        case DEFIPATH_VIAROTATION:
                                                ...
                                                break;
                                        case DEFIPATH_RECT:
                                                int deltaxl, deltayl, deltaxu,
                                                    deltayu;
                                                path->getViaRect(
                                                    &deltaxl, &deltayl,
                                                    &deltaxu, &deltayu);
                                                point->clsHasRectangle = true;
                                                point->clsRect[LOWER][X] =
                                                    static_cast<DBU>(deltaxl);
                                                point->clsRect[LOWER][Y] =
                                                    static_cast<DBU>(deltayl);
                                                point->clsRect[UPPER][X] =
                                                    static_cast<DBU>(deltaxu);
                                                point->clsRect[UPPER][Y] =
                                                    static_cast<DBU>(deltayu);
                                        ...
                                        case DEFIPATH_VIRTUALPOINT:
                                                ...
                                                // int x, y;
                                                // path->getVirtualPoint(&x,
                                                // &y);
                                                // std::cout<<"virtualPoint:
                                                // "<<netDscp.clsName<<"\n";
                                                break;
                                        case DEFIPATH_WIDTH:
                                                // only for special nets
                                                //						segmentDscp.clsWidth
                                                //= path->getWidth();
                                                break;
                                        case DEFIPATH_POINT:
                                                path->getPoint(&x, &y);
                                                segmentDscp.clsRoutingPoints
                                                    .push_back(
                                                        DefRoutingPointDscp());
                                                point = &segmentDscp
                                                             .clsRoutingPoints
                                                             .back();
                                                point->clsPos =
                                                    DBUxy(static_cast<DBU>(x),
                                                          static_cast<DBU>(y));
                                                point->clsExtension = -1;

                                                break;
                                        case DEFIPATH_FLUSHPOINT:
                                                path->getFlushPoint(&x, &y,
                                                                    &extension);
                                                segmentDscp.clsRoutingPoints
                                                    .push_back(
                                                        DefRoutingPointDscp());
                ...
                for (int j = 0; j < wire->numPaths(); j++) {
                        ...
                        while (pathId != DEFIPATH_DONE) {
                                switch (pathId) {
                                        ...
                                        case DEFIPATH_RECT:
                                                //						int
                                                // deltaxl, deltayl, deltaxu,
                                                // deltayu;
                                                //						path->getViaRect(&deltaxl,
                                                //&deltayl, &deltaxu, &deltayu);
                                                //						segmentDscp.clsRect[LOWER][X]
                                                //= static_cast<DBU>(deltaxl);
                                                //						segmentDscp.clsRect[LOWER][Y]
                                                //= static_cast<DBU>(deltayl);
                                                //						segmentDscp.clsRect[UPPER][X]
                                                //= static_cast<DBU>(deltaxu);
                                                //						segmentDscp.clsRect[UPPER][Y]
                                                //= static_cast<DBU>(deltayu);
                                                //						segmentDscp.clsHasRectangle
                                                //= true;
                                                break;
                                        case DEFIPATH_VIRTUALPOINT:
                                                // TODO
                                        ...
                                        case DEFIPATH_TAPERRULE:
                                                ...
                                                std::cout
                                                    << "TODO "
                                                       "DEFIPATH_TAPERRULE at "
                                                    << __func__ << "\n";
                                                break;
                                        case DEFIPATH_STYLE:
                                                // path->getStyle();
                                                std::cout
                                                    << "TODO DEFIPATH_STYLE at "
                                                    << __func__ << "\n";
                                                break;
                                        case DEFIPATH_SHAPE:
                                                // Not being read yet
...
int defVia(defrCallbackType_e c, defiVia* via, defiUserData ud) {
        ...
        if (via->hasViaRule()) {
                ...
                char* name;
                int xSize, ySize, xCutSpacing, yCutSpacing, xBotEnc, yBotEnc,
                    xTopEnc, yTopEnc;
                char* bottom;
                char* cut;
                char* top;
                via->viaRule(&name, &xSize, &ySize, &bottom, &cut, &top,
                             &xCutSpacing, &yCutSpacing, &xBotEnc, &yBotEnc,
                             &xTopEnc, &yTopEnc);
                ...
                if (via->hasRowCol()) {
                        ...
                        via->rowCol(&viaDscp.clsNumCutRows,
                ...
                if (via->hasOffset()) {
                        ...
                        via->offset(&xBotOffset, &yBotOffset, &xTopOffset,
                                    &yTopOffset);
                ...
                std::map<std::string, std::deque<DefViaGeometryDscp>>& mapGeos =
                    viaDscp.clsGeometries;
                int xl, yl, xh, yh;
                char* layerName;
...
int defGCellGrid(defrCallbackType_e typ, defiGcellGrid* gCell,
        ...
        // cout<<" # "<<designGCellGrid.master <<  " " <<designGCellGrid.doStart
        // <<" "<< designGCellGrid.doCount<<
...
void DEFControlParser::writeDEF(
    const std::string& filename, const std::string designName,
    const std::vector<DefComponentDscp>& components) {
        ...
        for (const DefComponentDscp& comp : components) {
                // int defwComponent(const char* name, const char* master, const
                // char* eeq, const char* source, const char* status, int
                // statusX, int statusY, int statusOrient, double weight, const
                // char* region);
                status = defwComponent(
                    comp.clsName.c_str(), comp.clsMacroName.c_str(), 0, NULL,
...
void DEFControlParser::writeFullDEF(const string& filename,
        ...
        if (defDscp.clsHasDieBounds) {
                status = defwDieArea((int)defDscp.clsDieBounds[LOWER][X],
        ...
        for (const DefRowDscp& defRow : defDscp.clsRows) {
                status =
                    defwRow(defRow.clsName.c_str(), defRow.clsSite.c_str(),
                            defRow.clsOrigin[X], defRow.clsOrigin[Y],
                            defOrient(defRow.clsOrientation), defRow.clsNumX,
                            defRow.clsNumY, defRow.clsStepX, defRow.clsStepY);
                CHECK_STATUS(status);
        }  // end for
        ...
        for (const DefTrackDscp& track : defDscp.clsTracks) {
                ...
                status = defwTracks(track.clsDirection.c_str(),
                                    track.clsLocation, track.clsNumTracks,
                                    track.clsSpace, numLayers, layers);
                CHECK_STATUS(status);
                free((char*)layers);
        }  // end for
        status = defwNewLine();
        ...
        for (const DefViaDscp& via : defDscp.clsVias) {
                ...
                if (via.clsHasViaRule) {
                        status = defwViaViarule(
                            via.clsViaRuleName.c_str(), via.clsXCutSize,
                            via.clsYCutSize, via.clsBottomLayer.c_str(),
                            via.clsCutLayer.c_str(), via.clsTopLayer.c_str(),
                            via.clsXCutSpacing, via.clsYCutSpacing,
                            via.clsXBottomEnclosure, via.clsYBottomEnclosure,
                            via.clsXTopEnclosure, via.clsYTopEnclosure);
                        CHECK_STATUS(status);

                        if (via.clsHasRowCol) {
                                status = defwViaViaruleRowCol(
                                    via.clsNumCutRows, via.clsNumCutCols);
                                CHECK_STATUS(status);
                        }  // end if
                        if (via.clsHasOrigin) {
                                status = defwViaViaruleOrigin(
                                    via.clsXOffsetOrigin, via.clsYOffsetOrigin);
                                CHECK_STATUS(status);
                        }  // end if
                        if (via.clsHasOffset) {
                                status = defwViaViaruleOffset(
                                    via.clsXBottomOffset, via.clsYBottomOffset,
                                    via.clsXTopOffset, via.clsYTopOffset);
                                CHECK_STATUS(status);
                        }  // end if
                } else {
                        const std::map<std::string,
                                       std::deque<DefViaGeometryDscp>>&
                            mapGeos = via.clsGeometries;
                        for (auto& pair : mapGeos) {
                                const std::string layer = pair.first;
                                const std::deque<DefViaGeometryDscp>&
                                    geometries = pair.second;
                                for (size_t i = 0; i < geometries.size(); i++) {
                                        const DefViaGeometryDscp& geo =
                                            geometries[i];
                                        if (geo.clsHasMask) {
                                                defwViaRect(
                                                    layer.c_str(),
        ...
        if (numComponents > 0) {
                ...
                for (const DefComponentDscp& comp : defDscp.clsComps) {
                        // int defwComponent(const char* name, const char*
                        // master, const char* eeq, const char* source, const
                        // char* status, int statusX, int statusY, int
                        // statusOrient, double weight, const char* region);
                        int orient = defOrient(comp.clsOrientation);
                        status = defwComponent(
                            comp.clsName.c_str(), comp.clsMacroName.c_str(), 0,
                            NULL, NULL, NULL, NULL, NULL, 0, NULL, NULL, NULL,
                            NULL, comp.clsIsFixed ? "FIXED" : comp.clsIsPlaced
                                                                  ? "PLACED"
                                                                  : "UNPLACED",
                            (int)comp.clsPos[X], (int)comp.clsPos[Y], orient, 0,
                            NULL, 0, 0, 0, 0);
                        CHECK_STATUS(status);
                }  // end for
        ...
        if (numPins > 0) {
                ...
                for (const DefPortDscp& port : defDscp.clsPorts) {
                        status = defwPin(
                            port.clsName.c_str(), port.clsNetName.c_str(),
                            (port.clsSpecial
                                 ? 1
                                 : 0),  // 0 ignores, 1 set net as special
                            (port.clsDirection.compare(INVALID_DEF_NAME) == 0
                                 ? NULL
                                 : port.clsDirection.c_str()),
                            (port.clsUse.compare(INVALID_DEF_NAME) == 0
                                 ? NULL
                                 : port.clsUse.c_str()),
                            (port.clsLocationType.compare(INVALID_DEF_NAME) == 0
                                 ? NULL
                                 : port.clsLocationType.c_str()),
                            (int)port.clsPos[X], (int)port.clsPos[Y],
                            defOrient(port.clsOrientation),
                            (port.clsLayerName.compare(INVALID_DEF_NAME) == 0
                                 ? NULL
                                 : port.clsLayerName.c_str()),
        ...
        if (numSpecialNets > 0) {
                ...
                for (const DefSpecialNetDscp& defNet : defDscp.clsSpecialNets) {
                        ...
                        for (const DefNetConnection& defConn :
                             defNet.clsConnections) {
                                status = defwSpecialNetConnection(
                                    defConn.clsComponentName.c_str(),
                                    defConn.clsPinName.c_str(), 0);
                                CHECK_STATUS(status);
                        }  // end for
                        if (!defNet.clsWires.empty()) {
                                ...
                                CHECK_STATUS(status);
                                for (const DefWireDscp& wire :
                                     defNet.clsWires) {
                                        for (const DefWireSegmentDscp& segment :
                                             wire.clsWireSegments) {
                                                ...
                                                status =
                                                    defwSpecialNetPathLayer(
                                                        segment.clsLayerName
                                                            .c_str());
                                                CHECK_STATUS(status);

                                                status =
                                                    defwSpecialNetPathWidth(
                                                        segment.clsRoutedWidth);
                                                CHECK_STATUS(status);
                                                for (const DefRoutingPointDscp&
                                                         pt :
                        ...
                        if (defNet.clsUse != INVALID_DEF_NAME) {
                                status =
                                    defwSpecialNetUse(defNet.clsUse.c_str());
                                CHECK_STATUS(status);
                        }  // end if
