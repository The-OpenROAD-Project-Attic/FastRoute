...
#define FLUTE_JOIN(d, id0, id1)            \
        if (FLUTE_PARALLELIZE(d, id0)) {   \
                flute_threads[id0].join(); \
        }                                  \
        if (FLUTE_PARALLELIZE(d, id1)) {   \
                flute_threads[id1].join(); \
        }
...
FLUTE_DTYPE flutes_wl_MD(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[],
                         int acc);
FLUTE_DTYPE flutes_wl_RDP(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[],
                          int acc);
Tree flute(int d, FLUTE_DTYPE x[], FLUTE_DTYPE y[], int acc, int mapping[]);
Tree flutes_LD(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[]);
Tree flutes_MD(int id, int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[],
               int acc);
...
void readLUT() {
        ...
        if (fpwv == NULL) {
                ...
                printf(
                    ...
                    FLUTE_POWVFILE, FLUTE_PORTFILE);
                exit(1);
        }

#if FLUTE_FLUTEROUTING == 1
        fprt = fopen(FLUTE_PORTFILE, "r");
        if (fprt == NULL) {
                printf("Error in opening %s\n", FLUTE_PORTFILE);
                printf(
                    "Please make sure %s and %s\n(found in the Flute directory "
                    "of UMpack)\n"
                    "are in the current working directory.\n",
                    FLUTE_POWVFILE, FLUTE_PORTFILE);
                exit(1);
        ...
        for (d = 4; d <= FLUTE_D; d++) {
                ...
                for (k = 0; k < numgrp[d]; k++) {
                        ...
                        if (ns == 0) {  // same as some previous group
                                ...
                                fgetc(fpwv);  // '\n'
                                numsoln[d][k] = ns;
                                p = (struct csoln *)malloc(
                                    ns * sizeof(struct csoln));
                                LUT[d][k] = p;
                                for (i = 1; i <= ns; i++) {
                                        linep = (unsigned char *)fgets(
                                            (char *)line, 99, fpwv);
...
// Modified - Guilherme Flach - 21/Ago/2014 --------------------------------
        ...
        // Stores a thread for each core. Adds 1 to skip the dummy thread at
        // zero
        // index.
...
FLUTE_DTYPE flute_wl(int d, FLUTE_DTYPE x[], FLUTE_DTYPE y[], int acc) {
        ...
        if (d > FLUTE_MAXD) allocateSize = d + 1;
        FLUTE_DTYPE *xs =
            (FLUTE_DTYPE *)malloc(sizeof(FLUTE_DTYPE) * allocateSize);
        FLUTE_DTYPE *ys =
            (FLUTE_DTYPE *)malloc(sizeof(FLUTE_DTYPE) * allocateSize);
        ...
        if (d == 2)
                ...
                for (i = 0; i < d; i++) {
                        for (k = i + 1; ptp[k]->x == ptp[i]->x; k++)
                                if (ptp[k]->y ==
                                    ptp[i]->y)  // pins k and i are the same
                                        break;
...
FLUTE_DTYPE flutes_wl_LD(int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[]) {
        ...
        if (d <= 3)
                ...
                for (i = 3; i <= d - 1;
                     i++) {  // p0=0 always, skip i=1 for symmetry
                        pi = s[i];
                        for (j = d - 1; j > i; j--)
                                if (s[j] < s[i]) pi--;
                        k = pi + (i + 1) * k;
...
Tree flute(int d, FLUTE_DTYPE x[], FLUTE_DTYPE y[], int acc, int mapping[]) {
        ...
        if (d == 2) {
                ...
                for (i = 0; i < d; i++) {
                        ...
                        ptp[i] = &pt[i];
                        ptp[i]->o =
                            i;  // [NOTE] Added - Guilherme Flach - 17/Jun/2014
                }
...
// sort x
                ...
                if (d < FLUTE_SORTING_THRESHOLD) {
                        ...
                        for (i = 0; i < d - 1; i++) {
                                ...
                                minidx = i;
                                for (j = i + 1; j < d; j++) {
                                        // Modified - Guilherme Flach -
                                        // 28/Jul/2014
                                        // Using y as tie break so that FLUTE
                                        // result becomes independent of
                                        // input point ordering for large degree
                                        // nets where it is heuristic.
                                        if (minval > ptp[j]->x ||
                                            (minval == ptp[j]->x &&
                ...
                for (i = 0; i < d; i++) {
                        xs[i] = ptp[i]->x;
                        mapping[i] = ptp[i]->o;  // [NOTE] Added - Guilherme
                                                 // Flach - 17/Jun/2014
                        ptp[i]->o = i;
                }
...
// sort y to find s[]
                ...
                if (d < FLUTE_SORTING_THRESHOLD) {
                        ...
                        // Enabling this makes Flute to be faster, but the
                        // results differ
                        // from the evaluation script. Unless one changes Flute
                        // also inside
                        // the evaluation script and re-evaluate official
                        // results. However
                        // I think I tried this and for some reason was not
                        // possible.
                ...
                // Reverse the mapping. Up to now mapping is mapping tree point
                // indexes to
                // original point index, but we want the opposite. Is there a
                // way to avoid
                // the reversing? Using s[] as a temp array as it is not used
                // anymore.
                for (int i = 0; i < d; i++) s[i] = mapping[s[i]];
                for (int i = 0; i < d; i++) mapping[s[i]] = i;
        }
        free(xs);
        free(ys);
        free(s);
...
Tree flutes_MD(int id, int d, FLUTE_DTYPE xs[], FLUTE_DTYPE ys[], int s[],
        ...
        int i, r, p, maxbp, bestbp, bp, nbp, ub, lb, n1, n2, nn1, nn2, ms,
            newacc;
        ...
        if (s[0] < s[d - 1]) {
                ...
                if (ms <= d - 3) {
                        ...
                        FLUTE_FORK(d, id0, t1 = flutes_LMD(id0, ms + 2, x1, y1,
                                                           s1, acc));
                        FLUTE_FORK(d, id1, t2 = flutes_LMD(id1, d - ms, xs + ms,
                                                           ys + ms, s2, acc));
                ...
                if (ms >= 2) {
                        ...
                        FLUTE_FORK(d, id0, t1 = flutes_LMD(id0, d + 1 - ms, x1,
                                                           y1, s1, acc));
                        FLUTE_FORK(d, id1,
                                   t2 = flutes_LMD(id1, ms + 1, xs,
                                                   ys + d - 1 - ms, s2, acc));
                        FLUTE_JOIN(d, id0, id1);
...
#define BreakInX(bp) ((bp) % 2 == 0)
                ...
                if (BreakInX(maxbp)) {  // break in x
                        ...
                        FLUTE_FORK(d, id0, t1 = flutes_LMD(id0, p + 1, xs, y1,
                                                           s1, newacc));
                        FLUTE_FORK(d, id1, t2 = flutes_LMD(id1, d - p, xs + p,
                                                           y2, s2, newacc));
                        FLUTE_JOIN(d, id0, id1);
                        ...
                        FLUTE_FORK(d, id0, t1 = flutes_LMD(id0, p + 1, x1, ys,
                                                           s1, newacc));
                        FLUTE_FORK(d, id1, t2 = flutes_LMD(id1, d - p, x2,
                                                           ys + p, s2, newacc));
                        FLUTE_JOIN(d, id0, id1);
...
/*
double calcIllegality(const vector< pair<Point,Point> > &tempEdges, const
vector<BBox> &obs, const vector<unsigned> &relevantObs)
{
   ...
   for(unsigned i = 0; i < tempEdges.size(); ++i)
   {
         ...
         for(unsigned j = 0; j < relevantObs.size(); ++j)
         {
           ...
           if(lessThanDouble(tempEdges[i].first.y, obs[relevantObs[j]].yMax) &&
                  greaterThanDouble(tempEdges[i].first.y,
obs[relevantObs[j]].yMin) &&
                  lessThanDouble(xMin, obs[relevantObs[j]].xMax) &&
                  greaterThanDouble(xMax, obs[relevantObs[j]].xMin))
           {
                 // x segment of x first path fails
                 xFirstIntersectionLen += min(xMax, obs[relevantObs[j]].xMax) -
max(xMin, obs[relevantObs[j]].xMin);
           }
           if(lessThanDouble(tempEdges[i].second.x, obs[relevantObs[j]].xMax) &&
                  greaterThanDouble(tempEdges[i].second.x,
obs[relevantObs[j]].xMin) &&
                  lessThanDouble(yMin, obs[relevantObs[j]].yMax) &&
                  greaterThanDouble(yMax, obs[relevantObs[j]].yMin))
           {
                 // y segment of x first path fails
                 xFirstIntersectionLen += min(yMax, obs[relevantObs[j]].yMax) -
max(yMin, obs[relevantObs[j]].yMin);
           }

           // test the y first path
           if(lessThanDouble(tempEdges[i].second.y, obs[relevantObs[j]].yMax) &&
                  greaterThanDouble(tempEdges[i].second.y,
obs[relevantObs[j]].yMin) &&
                  lessThanDouble(xMin, obs[relevantObs[j]].xMax) &&
                  greaterThanDouble(xMax, obs[relevantObs[j]].xMin))
           {
                 // x segment of y first path fails
                 yFirstIntersectionLen += min(xMax, obs[relevantObs[j]].xMax) -
max(xMin, obs[relevantObs[j]].xMin);
           }
           else if(lessThanDouble(tempEdges[i].first.x,
obs[relevantObs[j]].xMax) &&
                           greaterThanDouble(tempEdges[i].first.x,
obs[relevantObs[j]].xMin) &&
                           lessThanDouble(yMin, obs[relevantObs[j]].yMax) &&
                           greaterThanDouble(yMax, obs[relevantObs[j]].yMin))
           {
                 // y segment of y first path fails
                 yFirstIntersectionLen += min(yMax, obs[relevantObs[j]].yMax) -
max(yMin, obs[relevantObs[j]].yMin);
           }
         }
...
/*
Tree flautist(int d, DTYPE x[], DTYPE y[], int acc, const vector<BBox> &obs,
const vector<unsigned> &relevantObs, unsigned &legal)
{
   ...
   // build a tree with no additional points and see if it intersects any
obstacles
   ...
   unsigned maxAdditionalPoints = max(4*relevantObs.size(),
static_cast<unsigned>(MAXD - d));
   ...
   set<Point> newPoints;
   for(set<Point>::const_iterator i = oldPoints.begin(); i != oldPoints.end();
++i)
   {
         BBox basebox;
         basebox += *i;
         for(unsigned j = 0; j < relevantObs.size(); ++j)
         {
           ...
           if(oldPoints.find(newPoint) == oldPoints.end())
...
newPoints.insert(newPoint);
   ...
   for(unsigned addedPoints = 1; addedPoints <= maxAdditionalPoints;
++addedPoints)
   {
         Point addedPoint(DBL_MAX,DBL_MAX);

         for(set<Point>::const_iterator j = newPoints.begin(); j !=
newPoints.end(); ++j)
         {
