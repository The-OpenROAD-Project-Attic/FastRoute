...
static int isa_formula(char *str) {
	   ...
	   have a general parser for arithmetic expressions; constant arithmetic expressions? */
        ...
        while (*p) {
                if ((*p >= '0' && *p <= '9') || *p == ' ' || *p == '\t' || *p == '*' || *p == '/' || *p == '+' || *p == '-' || *p == 'e' || *p == 'E' || *p == '.')
                        p++;
...
static void check_ff_bank_widths(si2drGroupIdT group) /* called for ff_banks */
{
        ...
        if (!si2drObjectIsNull(attr, &err)) {
                ...
                if ((si2drSimpleAttrGetValueType(attr, &err) != SI2DR_STRING && si2drSimpleAttrGetValueType(attr, &err) != SI2DR_EXPR) || (si2drSimpleAttrGetValueType(attr, &err) == SI2DR_EXPR && e->valuetype != SI2DR_STRING)) {
...
static void check_vector(si2drGroupIdT group) {
        ...
        if (vd->dimensions > 2) {
                ...
                for (i = 1; i < vd->dim_sizes[2]; i++) {
                        if ((!neg && vd->values[i] > vd->values[0]) || (neg && vd->values[i] < vd->values[0])) {
                                found = 1;
                                break;
                        }
                }
                if (!found) {
                        ...
                        sprintf(EB, "%s:%d, The vector has its peak value (%g) at the beginning",
                                si2drObjectGetFileName(group, &err) ? si2drObjectGetFileName(group, &err) : "",
...
static void check_output_current_template(si2drGroupIdT group) {
	   ...
	   NEED TO CHECK THE NOMINAL VOLTAGES */
        ...
        if (!operating_conds_checked) {
                ...
                if (!si2drObjectIsNull(defopcond, &err) && si2drAttrGetAttrType(defopcond, &err) == SI2DR_SIMPLE && si2drSimpleAttrGetValueType(defopcond, &err) == SI2DR_STRING) {
                        ...
                        if (opcondname && *opcondname) {
                                ...
                                if (!si2drObjectIsNull(opcondg, &err)) {
                                        ...
                                        if (!si2drObjectIsNull(opcondvoltage, &err) && si2drAttrGetAttrType(opcondvoltage, &err) == SI2DR_SIMPLE && si2drSimpleAttrGetValueType(opcondvoltage, &err) == SI2DR_FLOAT64) {
                                                si2drFloat64T opcondvoltval = si2drSimpleAttrGetFloat64Value(opcondvoltage, &err);

                                                /* we now have one value, let's get the other */
                                                if (!si2drObjectIsNull(nomvolt, &err) && si2drAttrGetAttrType(nomvolt, &err) == SI2DR_SIMPLE && si2drSimpleAttrGetValueType(nomvolt, &err) == SI2DR_FLOAT64) {
...
static void check_level_shift(si2drGroupIdT group) {
        ...
        if (!si2drObjectIsNull(ivr, &err) && si2drObjectIsNull(ovr, &err) || si2drObjectIsNull(ivr, &err) && !si2drObjectIsNull(ovr, &err)) {
                si2drStringT df = si2drObjectGetFileName(ivr, &err);
                int dl = si2drObjectGetLineNo(ivr, &err);
...
static void check_ref_times(si2drGroupIdT group) {
        ...
        while (!si2drObjectIsNull((subg = si2drIterNextGroup(subs, &err)), &err)) {
                ...
                if (gt == LIBERTY_GROUPENUM_vector) {
                        ...
                        if (!si2drObjectIsNull(refattr, &err)) {
                                if (si2drAttrGetAttrType(refattr, &err) == SI2DR_SIMPLE && si2drSimpleAttrGetValueType(refattr, &err) == SI2DR_FLOAT64) {
					   ...
					   transition/slew  val */
                                        ...
                                        if (!si2drObjectIsNull(rg2, &err)) {
                                                ...
                                                if (si2drAttrGetAttrType(var1, &err) == SI2DR_SIMPLE && si2drSimpleAttrGetValueType(var1, &err) == SI2DR_STRING) {
                                                        si2drStringT varval = si2drSimpleAttrGetStringValue(var1, &err);
                                                        if (strstr(varval, "transition") || strstr(varval, "slew")) {
                                                                transvar = 1;
                                                        }
                                                ...
                                                if (si2drAttrGetAttrType(var2, &err) == SI2DR_SIMPLE && si2drSimpleAttrGetValueType(var2, &err) == SI2DR_STRING) {
                                        ...
                                        for (i = 0; i < refvals.items; i++) {
                                                if (!strncasecmp(refvals.template[i], nam, 200) && refvals.val[i] == val && (refvals.translew[i] == translewval)) {
                                                        found = 1;
                                                        break;
                                                }
                                        }
        ...
        for (i = 0; i < refvals.items; i++) {
                ...
                for (j = i + 1; j < refvals.items; j++) {
                        ...
                        if (strcasecmp(refvals.template[i], refvals.template[j]) == 0 && refvals.translew[i] == refvals.translew[j]) {
...
void check_ccs_density(si2drGroupIdT group) {
        ...
        while (!(si2drObjectIsNull((rg4 = si2drIterNextGroup(vectors, &err)), &err))) {
                ...
                for (j = 0; j < i; j++) {
                        if (vl_oc[j].x == vdv->index_info[0][0] && vl_oc[j].y == vdv->index_info[1][0]) {
                                found = 1;
                                break;
                        }
                }
                        ...
                        while (si2drIterNextComplexValue(vals, &type, &intgr, &float64, &string, &bool, &expr, &err), type != SI2DR_UNDEFINED_VALUETYPE) {
                                ...
                                if (arg && (arg->type == SYNTAX_ATTRTYPE_STRING && type != SI2DR_STRING)) {
                                        ...
                                        (*MsgPrinter)(SI2DR_SEVERITY_ERR, SI2DR_SYNTAX_ERROR,
                                                      ...
                                                      &err);
                                        errcount++;
                                } else if (arg && (arg->type == SYNTAX_ATTRTYPE_FLOAT && (type == SI2DR_STRING || type == SI2DR_BOOLEAN))) {
                                        if (type == SI2DR_STRING) {
                                ...
                                } else if (arg && (arg->type == SYNTAX_ATTRTYPE_INT && (type == SI2DR_STRING || type == SI2DR_BOOLEAN))) {
                                        if (type == SI2DR_STRING) {
                                                /* it's ok, if what's in the string is an int! */
