...
void print_strtab_stats(void) {
        printf("String Table Lookup called %d times\nString Table Enter called %d times\nNo. Strings entered into String Table: %d\n, Total Bytes entered: %d\n\n",
               total_lookup_calls, total_enter_calls, total_strings_entered, total_bytes_entered);
}
...
liberty_strtable *liberty_strtable_create_strtable(int numels, int strsize, int case_insensitive) {
        ...
        ht->hashtab = liberty_hash_create_hash_table(numels, 1 /* make it resizeable */, case_insensitive);
        htc = (liberty_strtable_chunk *)my_calloc(sizeof(liberty_strtable_chunk), 1);
        ht->chunklist = htc;
...
void liberty_strtable_destroy_strtable(liberty_strtable *ht) {
        ...
        if (ht->hashtab)
                liberty_hash_destroy_hash_table(ht->hashtab);
        ht->hashtab = 0;
        htc = ht->chunklist;
        while (htc) {
                htcn = htc->next;
...
char *liberty_strtable_enter_string(liberty_strtable *ht, char *str) {
        ...
        if (ht->chunklist->strtab_used + slen + 1 > ht->chunklist->strtab_allocated) {
                /* this string won't fit in the current chunk. realloc isn't an option, all the string pointers would
		   be off. So create a new chunk, and put it at the top of the list, and put the new string in there */
                liberty_strtable_chunk *htc = (liberty_strtable_chunk *)my_calloc(sizeof(liberty_strtable_chunk), 1);

                htc->strtab = (char *)my_calloc(ht->chunklist->strtab_allocated, 1);
        ...
        liberty_hash_enter_oid(ht->hashtab, strptr, t); /* ToDo: make a generic hashtab, then somehow use it as a base for oid hashes,
											  and then use it as a base for the string hash */
        return strptr;
}
