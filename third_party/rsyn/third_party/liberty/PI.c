...
si2drVoidT si2drPIQuit(si2drErrorT *err) {
        ...
        if (l__iter_group_count != 0 || l__iter_name_count != 0 || l__iter_val_count != 0 || l__iter_attr_count != 0 || l__iter_def_count != 0) {
                si2drErrorT err2;
                char tbuf[1000];
...
si2drBooleanT si2drObjectIsUsable(si2drObjectIdT object,
        ...
        *err = SI2DR_NO_ERROR;
        if (object.v2 == (void *)NULL)
                return SI2DR_FALSE;
        if (object.v1 == (void *)SI2DR_GROUP || object.v1 == (void *)SI2DR_ATTR || object.v1 == (void *)SI2DR_DEFINE)
                return SI2DR_TRUE;
        else
                return SI2DR_FALSE;
}
...
si2drVoidT si2drReadLibertyFile(char *filename,
        ...
        *err = SI2DR_NO_ERROR;
        ...
        if (strcmp(filename + strlen(filename) - 3, ".gz") && strcmp(filename + strlen(filename) - 2, ".Z") && strcmp(filename + strlen(filename) - 4, ".bz2") && strcmp(filename + strlen(filename) - 4, ".ZIP") && strcmp(filename + strlen(filename) - 4, ".zip"))
...
void lib__write_group(FILE *of, si2drGroupIdT group, char *indent, char *cellname) {
        ...
        for (attr = si2drIterNextAttr(attrs, &err);
             ...
             attr = si2drIterNextAttr(attrs, &err)) {
                ...
                if (at == SI2DR_SIMPLE &&
                    (!strcmp(nam, "default_operating_conditions") || !strcmp(nam, "default_wire_load") || !strcmp(nam, "default_wire_load_selection")))
                        continue;
...
si2drStringT si2drExprToString(si2drExprT *expr,
                               si2drErrorT *err) {
        *err = SI2DR_NO_ERROR;
        if (expr->type != SI2DR_EXPR_VAL && expr->type != SI2DR_EXPR_OP_ADD && expr->type != SI2DR_EXPR_OP_SUB && expr->type != SI2DR_EXPR_OP_MUL && expr->type != SI2DR_EXPR_OP_DIV && expr->type != SI2DR_EXPR_OP_PAREN && expr->type != SI2DR_EXPR_OP_LOG2 && expr->type != SI2DR_EXPR_OP_LOG10 && expr->type != SI2DR_EXPR_OP_EXP) {
                *err = SI2DR_INVALID_OBJECTTYPE;
                return "";
        }
...
si2drExprT *si2drOpExprGetRightExpr(si2drExprT *expr,
        ...
        *err = SI2DR_NO_ERROR;
        ...
        if (expr->type != SI2DR_EXPR_OP_ADD && expr->type != SI2DR_EXPR_OP_SUB && expr->type != SI2DR_EXPR_OP_MUL && expr->type != SI2DR_EXPR_OP_DIV && expr->type != SI2DR_EXPR_OP_EXP) {
                *err = SI2DR_INVALID_OBJECTTYPE;
                return 0;
        }
        return expr->right;
